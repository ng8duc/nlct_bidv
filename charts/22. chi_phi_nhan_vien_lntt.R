df <- full_join(
  df_chi_phi_nhan_vien %>% rename(chi_phi_nhan_vien := value),
  df_loi_nhuan_truoc_thue %>% rename(loi_nhuan_truoc_thue := value),
  by = c("yq", "name")
) %>% 
...
df <- df %>% 
  mutate(value = chi_phi_nhan_vien/loi_nhuan_truoc_thue*100)
...
chart_chi_phi_nhan_vien_lntt <- highchart() %>% 
  hc_yAxis(
    title = list(text = "%"),
  ...
  hc_title(
    align = "center",
    text = "Chi phí nhân viên/Lợi nhuận trước thuế của 11 NHTM quy mô lớn",
