df <- df_lai_du_thu %>% 
  ...
  full_join(df_thu_nhap_lai_cho_vay %>% rename(thu_nhap_lai_vay := value), join_by("yq", "name"))
...
df <- df %>% 
  mutate(value = lai_du_thu/thu_nhap_lai_vay)
...
chart_lai_du_thu_thu_nhap_lai <- highchart() %>% 
  hc_yAxis(
    title = list(text = "%"),
  ...
  hc_title(
    align = "center",
    text = "Tỷ lệ lãi dự thu/thu nhập lãi từ cho vay của 11 NHTM quy mô lớn",
